...
function openDrawer(triggerId) {
    ...
    // Section 2: Problème Identifié
    const section2 = createDrawerSection('PROBLÈME IDENTIFIÉ');
    ...
    // Section 3: Solution Implémentée
    const section3 = createDrawerSection('SOLUTION IMPLÉMENTÉE (RCA)');
    const solutionP = document.createElement('p');
    solutionP.textContent = details.solution;
    section3.appendChild(solutionP);
...
function toggleHighContrast() {
    ...
    localStorage.setItem('highContrastMode', AppState.highContrastMode);

    console.log(`[Theme] Mode haute contraste: ${AppState.highContrastMode ? 'ACTIVÉ' : 'DÉSACTIVÉ'}`);
}
...
function setupEventListeners() {
    ...
    if (themeToggleBtn) {
        themeToggleBtn.addEventListener('click', toggleHighContrast);
    }
